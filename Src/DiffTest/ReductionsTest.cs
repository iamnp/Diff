...
return sum/selection.Length;");
        ...
        public void TestStaticMeanReduction()
        {
            var gs = new GlobalScope();
            ...
            var interval = new GlobalScope.SearchInterval {Start = 10, End = 20};
...
return sum/selection.Length;");
            ...
            Assert.IsTrue(autoResetEvent.WaitOne());
            Assert.IsNotNull(reduction);
            Assert.AreEqual(1.0, reduction.Perform(interval, gs.AssignmentStatements[0]));
        }
    }
}